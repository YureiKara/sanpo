...
def render_news_column(region, feeds):
    ...
    if not all_items:
        ...
        for i, item in enumerate(all_items[:30]):
            ...
            title_el = f"<a href='{item['url']}' target='_blank' style='color:{_link_c};text-decoration:none;font-size:12px;font-weight:500;line-height:1.4'>{item['title']}</a>" if item['url'] else f"<span style='color:{_link_c};font-size:12px;line-height:1.4'>{item['title']}</span>"
            meta_parts = []
            ...
            html += (
                ...
                f"<div style='font-size:10px;margin-bottom:3px;display:flex;gap:4px;align-items:baseline'>{meta_html}</div>"
                f"{title_el}</div>"
            )
    html += "</div>"
...
def render_news_tab(is_mobile):
    if is_mobile:
        ...
        regions = list(NEWS_FEEDS.items())
        row1 = st.columns(2)
        for col, (region, feeds) in zip(row1, regions[:2]):
            with col:
                render_news_column(region, feeds)
        row2 = st.columns(2)
        for col, (region, feeds) in zip(row2, regions[2:]):
            with col:
                render_news_column(region, feeds)
